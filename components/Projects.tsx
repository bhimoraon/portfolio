      ...
      <div className="proj border-t-[1px] border-zinc-500">
        <div className="flex gap-10 justify-center relative items-center pt-20 px-20 ">
          <div className="absolute top-[45%] left-[40%] h-10 bg-black -translate-y-[50%] -translate-x-[50%]">
            <motion.div onMouseEnter={()=>setfirst("visible")} onMouseLeave={()=>setfirst("hidden")} className={`absolute ${first}    font-extrabold text-green-600 text-[7vw]`}   >
              FYDE
            </motion.div>
            <motion.div  onMouseEnter={()=>setfirstt("visible")} onMouseLeave={()=>setfirstt("hidden")} className={`absolute ${firstt}   font-extrabold text-green-600 text-[7vw]`}>
              VISE
            </motion.div>
          </div>

          <div onMouseEnter={()=>{setfirst("visible")}} onMouseLeave={()=>setfirst("hidden")}  className="left w-1/2 gap-10  ">
          ...
          <div onMouseEnter={()=>{setfirstt('visible')}} onMouseLeave={()=>setfirstt("hidden")} className="right w-1/2 gap-10 ">
            <span className="flex items-center gap-3 mb-5">
              <div className="w-3 h-3 rounded-full bg-white"></div>
              <h3>VISE</h3>
            </span>
